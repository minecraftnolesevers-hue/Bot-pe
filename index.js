...
function runBotApp() {
    ...
    // Sử dụng cơ chế hỗ trợ Bedrock qua Mineflayer
    bot = mineflayer.createBot({
        host: CONFIG.MC_HOST,
        port: CONFIG.MC_PORT,
        username: CONFIG.MC_USER,
        version: CONFIG.MC_VER,
        // Dòng quan trọng để fix lỗi Protocol cho PE
        profilesFolder: './nbt_data'
    });
    ...
    bot.on('end', () => {
        ...
        setTimeout(runBotApp, 15000);
    });
    bot.on('error', (err) => console.log('Lỗi:', err.message));
}

// 5. LỆNH DISCORD (NGỦ & STATUS)
    ...
    if (msg === '!sleep') {
        ...
        if (bed) {
            ...
            bot.once('goal_reached', async () => {
                ...
                catch (e) { message.reply('❌ Lỗi khi ngủ.'); }
            });
        }
    }
});
